"""从 ROS 2 rosbag 生成标签模板、分类报告和有限参数建议。

输入与输出
----------
工具读取低带宽 ``/vision/obstacle_evidence`` 与 ``/terrain/features``，把 CSV 人工真值按
rosbag 接收时间匹配，输出混淆矩阵、逐类 precision/recall/F1、macro-F1 和建议 YAML。
较早样本用于搜索，最新样本留作验证，避免同一数据既调参又评分。

能力边界
--------
自动搜索仅覆盖 ``vision_min_confidence`` 以及 ``step/climb/stop_threshold``。HSV、ROI、
蓝白横杆间距、地面分割、坑/墙/杆几何阈值和真实米制误差必须通过原始图像/点云回放逐组
调整。建议文件不会覆盖正式配置，也不能证明真机能够越障；完整流程和标签含义见根目录
``instruction.txt`` 第五节。
"""
