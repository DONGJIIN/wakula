...
**556 tests、0 errors、0 failures、0 skipped**，并提供一键启动、独立停止、对接检查和 CI。URDF 只用于 RViz 外形与
传感器 TF 占位，
不能视为运动学或整机控制已完成。
详细清单与开发顺序见 `quickstart.txt`。

2026-09-02 三入口联合复验修复了“长期原地补扫”的确定原因：当前 Gazebo RGB-D 安装在
`base_link` 前方存在不可观测近场，旧参数却要求该盲区必须有地面回波，导致
`INVALID DEPTH -> 无可用前沿 -> search_turn` 循环；同时 Nav2 生命周期成功响应可能刚好
超过旧 2 s 本地期限，仿真传送后的 odom 也可能尚未对齐下一帧语义提示。现在点云连续地面门
从经标定的 `x=0.90 m` 起检查到 `x=1.70 m`，生命周期响应期限为 5 s，仿真语义提示会等待
传送后的新 odom。以三个独立 launch 实跑，8/8 障碍在 154 s 内完成，约 156 s 返回起点并
进入 `COMPLETED`；这证明仿真上层识别、任务编排与返航闭环，不代表真机物理越障能力。

同日自由探索回归进一步修复了“看见障碍就原地转圈”的任务调度问题。普通
`robocon_field.launch.py` 本来只提供传感器和底盘，不提供 `TraverseObstacle` 控制器；任务层
现在会记录该障碍、设置空间冷却并立即继续选择代价地图前沿，不再把“控制器未安装”当作
物理越障失败。交接瞬间控制器消失时最多等待 2 秒；补扫最多 4 次（一圈），每次补扫后强制
重新开放平移覆盖；语义歧义只增加一次观察视角。隔离的非闪现三进程实跑约 30 秒净平移
1.65 m，连续选择约 2.64、2.50、2.37 m 的前沿目标。该证据验证的是“平移探索优先”，未接
真实越障 Action 时完成清单仍应为 0/8，不能据此宣称完成物理越障。
...
- `terrain_analyzer`：将点云转换到 `base_link`，以稳健高度栅格、连通域和异常原始回波支撑量
  ...
  也会在默认 `base_link x=0.90～1.70 m` 的 0.80 m 滚动安全前视内
  检查到 `min(x=1.70 m, 障碍前缘前一格)` 的接近走廊。`x=0.90 m` 是当前机器人前缘与
  Gazebo RGB-D 近场盲区共同标定出的检查起点，真机须用平地 rosbag 重标；远处盲带会在机器人接近并进入该窗口时
...
- `autonomous_mission`：从 `/map` 的已知—未知边界提取真实自由前沿，在 Nav2 处于活动状态
  ...
  的交接守卫未通过，则交替做一次 60° 有界换向，随后尝试 0.80 m 的普通 Nav2 安全移动来改变观察站，
  ...
  每次回访按 8/16/32/64 秒退避；到达观察位但没重新识别也不清零。若越障控制器在任务
  调度时离线，只记录障碍及位置并立即继续平移探索；服务在交接瞬间消失时最多等待 2 秒。
  整场预算默认 300 秒：240 秒工作截止时取消未完成的
  非返程 Nav2/Traverse 目标并开始返程；300 秒硬截止时锁住自主速度、取消所有仍在
  运行的 Action，并保留未完成清单，绝不伪报 `COMPLETED`。无新目标时最多补扫一圈；每次
  补扫成功都会重新开启自由区平移覆盖，禁止连续只提交旋转目标。
...
首次 Nav2 配置、插件加载和生命周期 bond 使用 5 s 墙钟看门狗；同一代次略晚但明确
`success=true` 的结果会被接受，已开始重试后的旧结果仍由代次隔离，避免对 active 节点重复 STARTUP。
...
旋转进展时会取消并重规划；
`/traverse_obstacle` 服务端离线时不会再到入口等待：任务记录障碍后立即继续前沿/覆盖探索；
仅服务在交接瞬间消失的竞态最多等待 2 秒。若前沿、覆盖目标和可回访障碍均耗尽，任务分
4 次转满一圈复查；每次转向后先重新尝试平移覆盖，仍无新证据才返回起点。
