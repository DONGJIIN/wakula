...
9 个 ROS 2 包可编译，245 项测试通过，并提供一键启动、独立停止、对接检查和 CI。URDF 只用于 RViz 外形与
传感器 TF 占位，
不能视为运动学或整机控制已完成。
详细清单与开发顺序见 `quickstart.txt`。

感知与规划节点会在创建订阅、定时器和 Action 客户端之前校验原始 ROS 参数，包括话题格式、
