...
对当前“2D SLAM + Nav2 + OpenCV + 深度点云越障确认”算法，优先采用下面的基础组合：

1. **一台顶置 2D 激光雷达**：负责稳定的平面建图、定位和 Nav2 避障。
2. **一台前置 RGB-D 双目相机**：RGB 供 OpenCV 识别颜色与外形，深度点云供高度、坡度、
   粗糙度和越障动作确认。
3. **机身 IMU、关节编码器和足端接触**：用于形成可靠 `/odom`；雷达不能替代里程计。

仅安装普通 RGB 相机时，OpenCV 仍能发布视觉提示，但没有可靠的真实高度和距离，系统会按
安全规则等待点云确认，不能完整执行自动越障。3D 雷达可替代深度相机提供点云，也可兼顾
二维 SLAM，但成本、重量、功耗和 RK3588 点云负载更高；首版真机不建议同时堆叠多台高带宽
传感器。
...
| 传感器 | 建议规格 | 选择理由 |
|---|---|---|
| 2D 激光雷达 | 270°～360°视场；有效量程至少 8～10 m；10 Hz 以上，优选 20～40 Hz；角分辨率不大于 0.5°；近距盲区不大于 0.10～0.15 m；直接发布 `LaserScan` | 低负载、平面扫描稳定，最适合当前 SLAM Toolbox 和 Nav2 主链 |
| RGB-D 相机 | 双目深度，优选全局快门；RGB 与深度可对齐；水平视场约 75°～95°、垂直视场约 50°～65°；最近深度不大于 0.20～0.30 m；0.25～5 m 内可用；至少 640×480、30 FPS；发布 `Image`、`CameraInfo` 和 `PointCloud2` | 同时满足 OpenCV 和地形点云，运动时比滚动快门更不易发生几何畸变 |
| 可选 3D 雷达 | 垂直视场至少约 30°；近距盲区不大于 0.30～0.50 m；10 Hz 以上；可稳定发布带时间戳 `PointCloud2` | 室外强光、低纹理或远距离环境更稳，但必须限制点数并配置点云到 `LaserScan` 的转换 |

室内比赛场景优先选 RGB-D 双目/主动双目；如果会在阳光直射下运行，应现场验证主动红外
深度的有效率，必要时改用室外性能更好的双目或 3D 激光雷达。采购前必须确认厂商提供
ROS 2 Jazzy 与 ARM64/RK3588 可用的驱动，而不是只确认 x86 Ubuntu 可以运行。

型号只作为验证规格和驱动生态的例子，不是项目绑定：Orbbec 官方 ROS 2 Wrapper 当前列出
Jazzy 和 Gemini 330 系列支持；RealSense D455 类设备提供宽视场与全局快门；Hokuyo
UST-10LX 类 2D 雷达提供 270°、10 m 和 25 ms 扫描。最终应按预算、供货、室内/室外、
防护等级和 RK3588 实测结果选型。

#### 结合当前 Wakula 尺寸的安装起点

以下尺寸均以**正常站立、平地状态**测量；`x` 向前、`y` 向左、`z` 向上。它们是首轮标定
起点，不是加工公差。安装后必须用实测值更新 URDF/静态 TF。

| 传感器 | 推荐安装位置与姿态 | 当前调试 URDF | 关键要求 |
|---|---|---|---|
| 2D 雷达 | 机身顶部横向中心；距地约 0.50～0.60 m；相对 `base_link` 建议 `x=0～+0.08 m`、`y≈0`、`z=+0.06～+0.16 m`；扫描面水平，俯仰/横滚误差尽量小于 0.5° | `(0, 0, +0.085 m)`，距地约 0.525 m，位于推荐范围 | 270°/360°扫描面不能被相机、提手、外壳或腿遮挡；不要装在会随步态摆动的部件上 |
| RGB-D 相机 | 机身最前端中心；距地约 0.40～0.50 m；相对 `base_link` 建议 `x=+0.26～+0.32 m`、`y≈0`、`z=-0.04～+0.06 m`；光轴向下俯约 10°～15°，支架预留 5°～20°可调范围 | `(+0.275, 0, +0.015 m)`，距地约 0.455 m，但当前模型未下俯 | 下沿应看到机器人前方约 0.4～0.6 m 的地面，上沿仍应看到限高杆；左右腿和线缆不能长期占据主要 ROI |
| 可选 3D 雷达 | 机身顶部中心或略靠前；距地约 0.50～0.65 m；通常保持水平，必要时仅下俯 0°～5° | 当前未单独建模 | 保证前下方有足够垂直视场；若与 2D 雷达共存，避免相互遮挡并控制总重心高度 |

当前 URDF 的 `base_link` 距名义地面 0.44 m。实际机器狗蹲伏、起伏和步态会改变传感器距地
高度，因此相机视场要留余量，不能只在静止站立时刚好看到台阶。相机太低容易被腿遮挡并
放大地面纹理；相机过高或完全水平会产生近场盲区；2D 雷达太低则会不断扫到摆动腿。

#### 机械、电气和标定要求

- 相机和雷达必须固定在刚性机身上；可使用薄型减振垫抑制高频振动，但不能使用会让外参
  随步态变化的软悬挂。支架应有防松、限位和可重复拆装定位。
- 镜头和雷达窗口前方不得有透明罩反光、保护杆或线缆；保留散热空间，并做拉力释放、供电
  余量和 USB/Ethernet 接头防松。室外使用时同时考虑防水、防尘和阳光直射。
- RGB 与深度优先使用同一设备的硬件对齐和同源时间戳；所有传感器与 RK3588 保持系统时间
  同步。不要用“当前时间”替换驱动采集时间。
- 用标定板完成相机内参和畸变标定；测量 `base_link -> camera_link`、
  `base_link -> lidar_link` 六自由度外参，建议初期争取做到平移误差不大于 5 mm、角度误差
  不大于 0.5°，再通过点云与真实平面重合情况复核。
- 在 RViz 中检查激光、点云、机器人模型和墙面/地面是否重合；分别以站立、行走振动、蹲伏
  和转弯状态录制 rosbag。确认外参后，再按 ROI → HSV → 点云高度阈值的顺序调算法。

相关官方资料：[ROS 2 Jazzy tf2 坐标约定](https://docs.ros.org/en/jazzy/p/tf2/generated/doxygen/html/index.html)、
...
[Hokuyo UST-10/20LX](https://www.hokuyo-aut.jp/search/single.php?serial=167)。
