...
| 强化学习训练环境 | ⬜ 未实现，等待真机 | 在独立 NVIDIA GPU 工作站上基于 Isaac Lab（运行于 Isaac Sim）搭建训练环境，设计观测空间、动作空间、奖励函数、终止条件及训练任务；不再以已停止支持的 Isaac Gym 作为新项目基线 |
...
4. **阶段 3：模型、状态估计和基础步态**
   ...
   - 工作：将真实机器人参数映射到 Isaac Lab/Isaac Sim 强化学习环境，配置质量、惯量、碰撞、摩擦、关节阻尼、执行器响应、PD 参数和控制频率。
...
- **强化学习采用 Isaac Lab，不再新建 Isaac Gym 工程。** NVIDIA 已将 Isaac Gym 标为
  legacy 且不再支持，并把 Isaac Lab 定义为后继方案。仓库当前还没有真实质量、惯量、
  执行器和关节参数，因此只更新后续路线，不在本阶段安装或伪造训练环境。Isaac Lab 训练
  应放在带 NVIDIA GPU 的开发机上，RK3588 仍负责 ROS 2 导航和后续策略推理，不承担训练。
